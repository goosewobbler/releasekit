...
- **Release-commit prefix**: commit subjects starting with `chore: release ` match `release.ci.skipPatterns` and suppress standing-PR update runs. Release commits must keep this prefix; nothing else should use it. Do not add `[skip ci]` to standing-PR commits — the squash merge inherits it and suppresses the publish workflow.
